...
if $DO_REHOST; then
    ...
    if [[ -d "$RESULT_PATH" ]]; then
        ...
        SECCOMP_PROFILE="$(cd "$(dirname "$0")" && pwd)/seccomp_no_reboot.json"
        SECCOMP_OPT=""
        ...
        if [[ -n "$CHKUP_HINTS_SRC" ]]; then
            [[ -d "$CHKUP_HINTS_SRC" ]] || die "ChkUp hints directory not found: $CHKUP_HINTS_SRC"
            log "Copying ChkUp hints into container at /gh/chkup_hints ..."
            docker exec "$CONTAINER" mkdir -p /gh/chkup_hints
        ...
        # 自動記錄結果到 CSV
        SAVE_SCRIPT="$(cd "$(dirname "$0")" && pwd)/save_result.sh"
        if [[ -f "$SAVE_SCRIPT" ]]; then
            bash "$SAVE_SCRIPT" "$RESULT_PATH" "${CHKUP_HINTS_SRC:-}" "$(basename "$FIRMWARE")" \
