#!/bin/bash
# Greenhouse batch rehosting for D-Link DSL series firmware.
# These are DSL modem/routers with MIPS-based Linux rootfs (uClibc + anweb).

set -euo pipefail

GREENHOUSE_IMAGE="greenhouse:patched"
BRAND="dlink"
OUTDIR="./results/batch_dsl"
MAX_CYCLES=26
PORTS="80,81"
IP="172.21.0.2"
REHOST_TYPE="HTTP"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help) echo "Usage: $0 [--outdir DIR] [--screenshot] [--screenshot-wait N]"; exit 0 ;;
        *) die "Unknown option: $1" ;;
    esac
done

STAGING_DIR=$(mktemp -d /tmp/gh_batch_XXXXXX)
log "Collecting DSL firmware from $FW_ROOT ..."

# DSL firmware directories have date-prefixed names containing "DSL"
find -L "$FW_ROOT" -maxdepth 2 -type f -iname "*.bin" \
    | grep -i "DSL" \
    | while read -r f; do
        cp "$f" "$STAGING_DIR/"
        log "  + $(basename "$f")"
...
[[ "$FILE_COUNT" -eq 0 ]] && { rm -rf "$STAGING_DIR"; die "No DSL firmware files found."; }
...
log "Patching Planter.py (anweb support + symlink fix)..."
